...
action :remove do
  data_bag("#{new_resource.data_bag}").each do |login|
    u = data_bag_item("#{new_resource.data_bag}", login)
    next unless (u['groups'].include?("#{new_resource.search_group}") and u['action'] == 'remove')
    user login['id'] do
      action :remove
    end
  end
